...
describe("OAuth 2 Credentials API", function()
  ...
  describe("/consumers/:consumer/oauth2/:id", function()
    ...
    describe("PATCH", function()
      ...
      it("[FAILURE] should return proper errors", function()
        local response, status = http_client.patch(BASE_URL..credential.id, {redirect_uri = "http://getkong.org,wrong_uri"})
        assert.equal(400, status)
        assert.equal('{"redirect_uri":"cannot parse \'wrong_uri\'"}\n', response)
      end)
    end)
